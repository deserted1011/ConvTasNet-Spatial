#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""最小示例：加载臂 B（目标模型）权重，对一条多通道 wav 出 [2, T]。

    python 代码/load_and_infer.py --mix <多通道 wav> --ref-channel 1 --out out_b.wav
    python 代码/load_and_infer.py --mix <wav> --spatial <xxx.space.npy> --out out_b.wav

注意：**不给 --spatial 就用全零特征**，此时输出与臂 A 逐元素相等（那正是消融口径
`--zero-spatial`），不代表「没有空间特征也能跑」。
"""
...
def main():
    ...
    ap.add_argument("--ckpt", default=os.path.join(ROOT, "ckpt_B_ep30.pt"))
    ap.add_argument("--device", default="cuda")
    ap.add_argument("--out", default="out_b.wav")
    args = ap.parse_args()
    ...
    model, info = load_arm("B", ckpt=args.ckpt, device=args.device)
    ...
    if args.spatial:
        ...
        print("[提示] 未给空间特征，用全零：输出等价于臂 A（消融口径）")

    y = infer(model, "B", x, sp, device=info["device"])
