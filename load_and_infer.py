...
"""load_and_infer.py —— 最小示例：50 行看懂「模型怎么加载、输入怎么喂」。
    ...
    python load_and_infer.py 三麦录音.wav --out out_b.wav
    python load_and_infer.py 三麦录音.wav --device cpu --out out_b.wav

它做的事（等价于 separate_B.py 的 1~4 步，但只跑一条、输出 float32 波形，便于二次处理）：
    读多通道 wav -> 取 ch1 当波形 -> 算 6 通道空间特征并 z-score -> 加载臂 B -> 前向 -> 写 wav

`--zero-spatial` 是**消融开关**：把空间特征全置 0 再喂进去。这时 bottleneck 新增的那
6 列接的是 0，输出与「只有波形的那条对照臂（臂 A）」逐元素相等 —— 这是本项目验证
「空间特征确实进了网络」的办法，不是"没有空间特征也能跑"的意思。
"""
...
from arm_models import infer, load_arm, n_frames_of   # noqa: E402
from separate_B import READ_WAV, WRITE_WAV, resample  # noqa: E402


def main():
    ap = argparse.ArgumentParser(description="臂 B 最小推理示例")
    ap.add_argument("input", help="多通道 wav（>= 3 通道；16 kHz 最佳，其它采样率会自动重采样）")
    ap.add_argument("--out", default="out_b.wav", help="输出 wav（float32，完整动态范围，不做峰值归一）")
    ap.add_argument("--config", default=os.path.join(HERE, "config.json"))
    ap.add_argument("--ckpt", default=os.path.join(HERE, "臂B_模型.pt"))
    ...
    ap.add_argument("--zero-spatial", action="store_true", help="消融：空间特征全置 0")
    args = ap.parse_args()

    x, sr = READ_WAV(args.input)
    if x.shape[0] < 3:
        raise SystemExit("输入只有 %d 个通道：臂 B 需要 >= 3（ch0/ch1/ch2 = 2 cm 间距线性三麦）" % x.shape[0])
    x, sr = resample(x, sr)

    model, info = load_arm("B", ckpt=args.ckpt, device=args.device or ("cuda" if _cuda() else "cpu"),
                           config=args.config)
    ...
    if args.zero_spatial:
        ...
        print("[消融] 空间特征全置 0：输出应与臂 A 逐元素相等")
    else:
    ...
    y = infer(model, "B", wav, sp, device=info["device"])
